		...
		<div class="w-full h-full rounded-lg bg-white shadow-md p-2 m-0">
			<x-form action="{{ route('solicitud.store') }}" formTitle="Formulario de Solicitud">
				...
				<x-fieldset label="Datos Fiscales" step="1">
					<x-input-solicitud name="actividad_economica" label="Actividad Economica" type="text" value="{{ old('str_activdad_economica', $datoFiscal->str_actividad_economica ?? '') }}"/>
					<x-input-solicitud name="regimen" label="Regimen" type="text" value="{{ old('str_regimen', $datoFiscal->str_regimen ?? '') }}" />
					<x-input-solicitud name="nombre_comercial" label="Nombre Comercial" type="text" value="{{ old('str_nombre_comercial', $datoFiscal->str_nombre_comercial ?? '') }}"/>
					<x-input-solicitud name="numero_empleados" label="Numero de Empleados" type="number" value="{{ old('int_numero_empleados', $datoFiscal->int_numero_empleados ?? '') }}"/>
					<x-input-solicitud name="razon_social" label="Razon Social" type="text" value="{{ old('str_razon_social', $datoFiscal->str_razon_social ?? '') }}" />
					<x-input-solicitud name="clave_imss" label="Clave IMSS" type="text" value="{{ old('str_clave_imss', $datoFiscal->str_clave_imss??'') }}"/>
					<x-input-solicitud name="clave_impi" label="Clave IMPI" type="text" value="{{ old('str_clave_impi', $datoFiscal->str_clave_impi??'') }}"/>
					<x-input-solicitud name="clave_affy" label="Clave AFFY" type="text" value="{{ old('str_clave_affy', $datoFiscal->str_clave_affy??'') }}"/>
					<x-input-solicitud name="clave_sat" label="Clave SAT" type="text" value="{{ old('str_clave_sat', $datoFiscal->str_clave_sat??'') }}"/>
					<x-input-solicitud name="clave_cif" label="Clave CIF" type="text" value="{{ old('str_clave_cif',$datoFiscal->str_clave_cif??'') }}"/>
				</x-fieldset>
				...
				<x-fieldset label="Domicilios" step="2">
					<x-slot name="dinamic_input">
						<x-dinamic-input :headers="['Direccion', 'Estado', 'Municipio', 'Localidad']" id_button="add_domicilio"
							id_message="message_domicilios_table_empty" id_table="domicilios_table" id_json="domicilios_json" value="{!! old('domicilios', $domicilios ?? '') !!}"
> 
				...
				<x-fieldset label="Productos" step="3">
					<x-slot name="dinamic_input">
						<x-dinamic-input :headers="['Nombre Producto', 'Descripcion', 'Produccion Mensual', 'Ventas Mensuales', 'Ventas Anuales']"
							id_button="add_producto" id_message="message_productos_table_empty" id_table="productos_table" id_json="productos_json" value="{!! old('productos', $productos ?? '') !!}">
				...
				<x-fieldset label="Redes Sociales" step="4">
					<x-slot name="dinamic_input">
						<x-dinamic-input :headers="['Red Social', 'Nombre', 'Enlace']" id_button="add_red_social" id_message="message_redes_sociales_table_empty" id_table="redes_sociales_table" id_json="redes_sociales_json" value="{!! old('redes_sociales', $redes_sociales ?? '') !!}">
